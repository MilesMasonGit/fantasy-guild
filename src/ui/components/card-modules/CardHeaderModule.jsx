...
const CardHeaderModule = React.memo(({ trait, card, isFirst, globalIndex = 0 }) => {
    ...
    const titleText = usesEnemyTitle ? enemyTitle : (card.name || template.name || 'Unknown Card');
    
    // Dynamic text sizing for long titles:
    // Drop from 20px (base) to 16px (sm) for titles over 12 characters.
    // If it's over 16 characters, drop to 14px.
    let titleSize = undefined;
    if (titleText.length > 16) {
        titleSize = '14px';
    } else if (titleText.length > 12) {
        titleSize = '16px';
    }
    ...
    return (
        <div className="flex flex-col relative z-10 w-full bg-black/30 py-2 px-3 rounded-t-xl border-b border-white/10">
            {/* Title */}
            <div className="flex flex-col items-center justify-center gap-1 mb-1 text-center relative w-full overflow-hidden">
                <span 
                    className="gi-card-title font-bold text-white tracking-widest uppercase"
                    style={titleSize ? { '--title-size': titleSize } : {}}
                >
                    {titleText}
                </span>
            </div>

import { useEntityStore } from '../../stores/useEntityStore';
import { useSimulationStore } from '../../stores/useSimulationStore';
import { 
  ArrowRight, Trash2, Sword, Activity, Lock, Unlock, Star, Percent, Coins,
  Package, Skull, Map, Scroll, Hammer, Tag, HelpCircle
} from 'lucide-react';
import EntitySelect from '../shared/EntitySelect';
import { resolveSpritePath } from '../../../../src/utils/AssetManager.js';

/**
 * SupplyChainColumn — The "Origin" (Left) or "Product" (Right) column for the editor.
 * Handles display and editing of linked entities.
 */
export default function SupplyChainColumn({ title, side, entities, onAdd, onUpdate, onRemove }) {
  const setActiveEntity = useEntityStore((s) => s.setActiveEntity);
  const items = useEntityStore((s) => s.items);
  const tasks = useEntityStore((s) => s.tasks);
  const recipes = useEntityStore((s) => s.recipes);
  const tags = useEntityStore((s) => s.tags || {});
  const itemUpdates = useSimulationStore((s) => s.itemUpdates) || {};
  const taskUpdates = useSimulationStore((s) => s.taskUpdates) || {};
  const recipeUpdates = useSimulationStore((s) => s.recipeUpdates) || {};
  const activeType = useEntityStore((s) => s.activeEntityType);
  const activeId = useEntityStore((s) => s.activeEntityId);

  const getEntityData = (id) => {
    if (!id) return null;
    if (items[id]) return { ...items[id], type: 'item', updates: itemUpdates[id] };
    if (tasks[id]) return { ...tasks[id], type: 'task', updates: taskUpdates[id] };
    if (recipes[id]) return { ...recipes[id], type: 'recipe', updates: recipeUpdates[id] };
    
    const state = useEntityStore.getState();
    if (state.enemies?.[id]) return { ...state.enemies[id], type: 'enemy', updates: useSimulationStore.getState().enemyUpdates?.[id] };
    if (state.lootTables?.[id]) return { ...state.lootTables[id], type: 'lootTable' };
    if (state.encounterTables?.[id]) return { ...state.encounterTables[id], type: 'encounterTable' };
    if (state.encounters?.[id]) return { ...state.encounters[id], type: 'encounter' };
    if (state.areas?.[id]) return { ...state.areas[id], type: 'area' };
    if (state.quests?.[id]) return { ...state.quests[id], type: 'quest' };
    if (state.workstations?.[id]) return { ...state.workstations[id], type: 'workstation' };
    return null;
  };

  // Determine which entity types can be added via the dropdown
  let selectableTypes = null;
  let showAddControls = true;

  if (activeType === 'area') {
    if (side === 'left') {
      showAddControls = false;
    } else {
      selectableTypes = ['area'];
    }
  } else if (activeType === 'quest') {
    if (side === 'left') {
      selectableTypes = ['item', 'enemy'];
    } else {
      selectableTypes = ['item']; // Quest rewards allow items (and gold via button)
    }
  } else if (activeType === 'workstation') {
    if (side === 'left') {
      selectableTypes = ['area'];
    } else {
      showAddControls = false; // Recipes list is read-only based on subskill
    }
  } else if (activeType === 'lootTable') {
    if (side === 'right') {
      selectableTypes = ['item'];
    } else {
      showAddControls = false; // Producers are calculated dynamically
    }
  } else if (activeType === 'encounter') {
    if (side === 'left') {
      selectableTypes = ['area'];
    } else {
      selectableTypes = ['enemy'];
    }
  } else if (activeType === 'encounterTable') {
    if (side === 'right') {
      selectableTypes = ['encounter'];
    } else {
      showAddControls = false;
    }
  } else {
    // Default/Standard behaviour for tasks, recipes, enemies, items
    selectableTypes = side === 'right' ? ['item', 'lootTable'] : ['item', 'tag'];
  }

  // Prevent showing search box if only 1 item is allowed and we already have it (e.g., workstation area)
  const isSingleSlot = (activeType === 'workstation' || activeType === 'encounter') && side === 'left';
  const hideSearchBox = !showAddControls || (isSingleSlot && entities.length >= 1);

  return (
    <div className="flex flex-col h-full bg-[#16161a] border-x border-white/5 w-80 shrink-0 overflow-hidden">
      <div className="p-3 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-500 flex items-center gap-2">
          {side === 'left' ? <Activity size={12} className="rotate-180" /> : <Activity size={12} />}
          {title}
        </h3>
      </div>

      {!hideSearchBox && (
        <div className="p-2 bg-black/20 border-b border-white/5 space-y-2">
           <div className="flex gap-2">
             <div className="flex-1">
               <EntitySelect 
                 entityTypes={selectableTypes}
                 onChange={(id, type) => {
                   if (id) onAdd(id, type);
                 }} 
                 placeholder={`Search or add ${selectableTypes.join('/')}...`}
               />
             </div>
             {side === 'left' && (activeType === 'task' || activeType === 'recipe') && (
               <select
                 onChange={(ev) => {
                   const val = ev.target.value;
                   if (val) {
                     onAdd(val, 'tag');
                     ev.target.value = '';
                   }
                 }}
                 className="bg-[#1f1f24] border border-white/10 rounded px-1.5 text-[10px] font-bold uppercase tracking-wider text-emerald-400 outline-none focus:border-emerald-500/50 cursor-pointer shrink-0"
                 style={{ maxWidth: '85px' }}
               >
                 <option value="">+ Tag...</option>
                 {Object.values(tags).map(t => (
                    <option key={t.id} value={t.id}>{t.name}</option>
                  ))}
               </select>
             )}
             {side === 'right' && (activeType === 'task' || activeType === 'recipe' || activeType === 'enemy') && (
               <button 
                 onClick={() => {
                   const encIndex = entities.findIndex(e => e.type === 'encounter');
                   if (encIndex !== -1) {
                     onRemove(encIndex);
                   } else {
                     onAdd(Date.now().toString(), 'encounter');
                   }
                 }}
                 className={`flex items-center justify-center shrink-0 w-8 h-8 rounded border transition-colors ${
                   entities.some(e => e.type === 'encounter') 
                     ? 'bg-rose-500/20 text-rose-400 border-rose-500/30 hover:bg-rose-500/30 hover:text-rose-300' 
                     : 'bg-white/5 text-gray-400 border-white/10 hover:bg-emerald-500/20 hover:text-emerald-400'
                 }`}
                 title={entities.some(e => e.type === 'encounter') ? "Remove Encounter Pool" : "Add Encounter Pool"}
               >
                 <Sword size={14} />
               </button>
             )}
             {side === 'right' && activeType === 'quest' && (
               <button 
                 onClick={() => onAdd('gold', 'gold')}
                 className="flex items-center justify-center shrink-0 w-8 h-8 rounded border bg-white/5 text-amber-400 border-white/10 hover:bg-amber-500/20 hover:border-amber-500/30 transition-colors"
                 title="Add Gold Reward"
               >
                 <Coins size={14} />
               </button>
             )}
           </div>
        </div>
      )}

      <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
        {entities.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center opacity-20 py-10">
            <div className="text-4xl mb-2">⛓️</div>
            <p className="text-[10px] uppercase font-bold tracking-tight">No links configured</p>
          </div>
        ) : entities.map((e, i) => {
          // Encounter layout (embedded deck list style)
          if (e.type === 'encounter' && activeType !== 'encounterTable') {
            const totalWeight = (e.enemies || []).reduce((sum, en) => sum + (en.weight || 100), 0);
            return (
              <div key={`enc-${i}`} className="group relative rounded-xl border border-rose-500/30 bg-rose-500/[0.02] p-3 space-y-3 overflow-visible">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-lg bg-rose-500/20 text-rose-400 flex items-center justify-center shrink-0">
                      <Sword size={14} />
                    </div>
                    <div>
                      <div className="text-xs font-bold text-rose-400 uppercase tracking-widest">Encounter Pool</div>
                      <div className="text-[10px] text-gray-500 leading-none">Triggers combat phase</div>
                    </div>
                  </div>
                  <button onClick={() => onRemove(i)} className="p-1.5 rounded-lg text-gray-600 hover:text-red-400 hover:bg-red-400/10 transition-colors">
                    <Trash2 size={12} />
                  </button>
                </div>
                
                <div className="flex items-center justify-between bg-black/40 p-2 rounded border border-white/5">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Trigger Chance</span>
                  <div className="flex items-center gap-1">
                    <input 
                      type="number" min="0" max="100" 
                      value={e.chance ?? 100} 
                      onChange={(ev) => onUpdate(i, { chance: Number(ev.target.value) })} 
                      className="w-14 text-xs font-bold bg-white/5 border border-white/10 h-6 px-1.5 rounded text-right outline-none focus:border-rose-500/50 transition-colors" 
                    />
                    <span className="text-xs font-bold text-gray-500">%</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest border-b border-white/5 pb-1">Enemy Drops</div>
                  <div className="space-y-1">
                    {(e.enemies || []).map((en, enIdx) => {
                      const enemyData = useEntityStore.getState().enemies[en.enemyId];
                      const realChance = totalWeight > 0 ? ((en.weight || 100) / totalWeight) * (e.chance ?? 100) : 0;
                      return (
                        <div key={enIdx} className="flex items-center gap-2 bg-black/20 p-1.5 rounded border border-white/5">
                          {(() => {
                            const enemyPath = enemyData ? resolveSpritePath(enemyData) : null;
                            if (enemyPath) {
                              const imgSrc = enemyPath.startsWith('/') ? enemyPath : `/${enemyPath}`;
                              return (
                                <div className="w-5 h-5 flex items-center justify-center overflow-hidden shrink-0 select-none rounded border border-white/5 bg-black/20">
                                  <img 
                                    src={imgSrc} 
                                    className="w-full h-full object-contain pixel-art" 
                                    alt="" 
                                    onError={(e) => { 
                                      e.target.style.display = 'none'; 
                                      if (e.target.nextElementSibling) e.target.nextElementSibling.style.display = 'flex'; 
                                    }} 
                                  />
                                  <Skull size={10} className="text-gray-500" style={{ display: 'none' }} />
                                </div>
                              );
                            }
                            return <Skull size={10} className="text-gray-500 shrink-0" />;
                          })()}
                          <span className="text-xs font-bold text-white truncate flex-1">{enemyData?.name || 'Unknown'}</span>
                          <div className="flex items-center gap-1 shrink-0">
                            <input 
                              type="number" 
                              value={en.weight || 100} 
                              onChange={(ev) => {
                                const newEnemies = [...(e.enemies || [])];
                                newEnemies[enIdx].weight = Number(ev.target.value);
                                onUpdate(i, { enemies: newEnemies });
                              }} 
                              className="w-12 text-xs font-bold text-right bg-white/5 border border-white/10 rounded h-5 px-1 outline-none focus:border-rose-500/50" 
                              title="Weight" 
                            />
                            <span className="text-[9px] text-gray-500 w-8 text-right font-mono">{realChance.toFixed(1)}%</span>
                            <button 
                              onClick={() => {
                                const newEnemies = [...(e.enemies || [])].filter((_, idx) => idx !== enIdx);
                                onUpdate(i, { enemies: newEnemies });
                              }} 
                              className="p-0.5 text-gray-600 hover:text-red-400 ml-1 transition-colors"
                            >
                              <Trash2 size={10}/>
                            </button>
                          </div>
                        </div>
                      );
                    })}
                  </div>
                  <div className="pt-1">
                    <EntitySelect 
                      entityTypes={['enemy']} 
                      placeholder="Add enemy to pool..." 
                      onChange={(enemyId) => {
                        const newEnemies = [...(e.enemies || []), { enemyId, weight: 100 }];
                        onUpdate(i, { enemies: newEnemies });
                      }}
                    />
                  </div>
                </div>
              </div>
            );
          }

          // Gold reward layout
          if (e.type === 'gold') {
            return (
              <div key={`gold-${i}`} className="group relative rounded-xl border border-amber-500/20 bg-amber-500/[0.02] p-3 space-y-3 overflow-hidden">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-black/40 flex items-center justify-center border border-white/5 shrink-0 text-amber-400">
                      <Coins size={18} />
                    </div>
                    <div>
                      <div className="text-sm font-black text-amber-400">Gold Coins</div>
                      <div className="text-[9px] text-gray-500 uppercase tracking-widest font-mono">Currency Reward</div>
                    </div>
                  </div>
                  <button onClick={() => onRemove(i)} className="p-1.5 rounded-lg text-gray-600 hover:text-red-400 hover:bg-red-400/10 transition-colors">
                    <Trash2 size={12} />
                  </button>
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest block">Amount (Gold GP)</label>
                  <input 
                    type="number"
                    min="1"
                    value={e.quantity || 100}
                    onChange={(ev) => onUpdate(i, { quantity: Number(ev.target.value) })}
                    className="w-full text-xs font-bold bg-black/40 border-white/5 h-8 px-2 rounded-lg focus:border-amber-500/50 outline-none text-amber-400"
                  />
                </div>
              </div>
            );
          }

          // Standard entity layout
          const id = e.id || e.itemId;
          let data = null;
          let updates = {};
          let isTagInput = false;

          if (e.tag) {
            isTagInput = true;
            const targetTag = e.tag.toLowerCase();
            const matchingItems = Object.values(items).filter(item => {
              const itemTags = (item.tags || []).map(t => t.toLowerCase());
              const itemType = (item.type || '').toLowerCase();
              return itemTags.includes(targetTag) || itemType === targetTag;
            });
            let minCost = 0;
            if (matchingItems.length > 0) {
              minCost = Math.min(...matchingItems.map(item => item.trueCost || 0));
            }
            data = {
              name: `Any ${e.tag}`,
              type: 'item',
              trueCost: minCost,
            };
          } else {
            data = getEntityData(id);
            updates = data?.updates || {};
          }

          // Star toggle logic
          let showStarToggle = false;
          let isPrimary = false;
          let handleStarToggle = null;

          if (!isTagInput) {
            if (side === 'right' && (activeType === 'task' || activeType === 'recipe' || activeType === 'enemy')) {
              const isItemType = e.type === 'item' || !e.type;
              if (isItemType) {
                showStarToggle = true;
                isPrimary = !!(e.isPrimarySource || e.isPrimaryOutput);
                handleStarToggle = () => onUpdate(i, { isPrimarySource: !isPrimary });
              }
            } else if (side === 'left' && activeType === 'item') {
              const state = useEntityStore.getState();
              const producerEntity = e.type === 'task' ? tasks[id] : (e.type === 'recipe' ? recipes[id] : state.enemies?.[id]);
              if (producerEntity) {
                showStarToggle = true;
                const outputsList = producerEntity.outputs || producerEntity.drops || [];
                const outputEntry = outputsList.find(o => (o.id || o.itemId) === activeId);
                isPrimary = !!(outputEntry?.isPrimarySource || outputEntry?.isPrimaryOutput);
                handleStarToggle = () => onUpdate(i, { isPrimarySource: !isPrimary });
              }
            }
          }

          // Check if inputs are editable for this item
          const isQuestRewards = activeType === 'quest' && side === 'right';
          const isQuestRequirements = activeType === 'quest' && side === 'left';
          const isLootTableEntries = activeType === 'lootTable' && side === 'right';
          const isEncounterEnemies = activeType === 'encounter' && side === 'right';
          const isEncounterTableEntries = activeType === 'encounterTable' && side === 'right';
          const isAreaChildren = activeType === 'area' && side === 'right';

          const showInputs = (side === 'left' || isQuestRewards || isLootTableEntries || isEncounterEnemies || isEncounterTableEntries || isAreaChildren) && activeType !== 'workstation';

          return (
            <div key={e.tag ? `tag-${e.tag}-${i}` : `${id}-${i}`} className="group relative rounded-xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] hover:border-white/20 transition-all p-3 space-y-3 overflow-hidden">
              {/* Header: Identity & Jump */}
              <div className="flex items-center justify-between gap-2 relative">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-lg bg-black/40 flex items-center justify-center border border-white/5 shrink-0">
                    {(() => {
                      if (e.tag) return <Tag size={18} className="text-gray-500" />;
                      
                      const spritePath = data ? resolveSpritePath(data) : null;
                      const isBg = data?.type === 'task' || data?.type === 'workstation' || data?.type === 'area';
                      const isEnemy = data?.type === 'enemy';

                      const typeIconMap = {
                        item: Package,
                        task: Sword,
                        recipe: Scroll,
                        enemy: Skull,
                        quest: Scroll,
                        workstation: Hammer,
                        area: Map,
                        lootTable: Package,
                        encounterTable: Sword,
                        encounter: Sword
                      };
                      const FallbackIcon = typeIconMap[data?.type] || HelpCircle;

                      if (spritePath) {
                        const imgSrc = spritePath.startsWith('/') ? spritePath : `/${spritePath}`;
                        return (
                          <div className="w-10 h-10 flex items-center justify-center overflow-hidden shrink-0 select-none rounded border border-white/5 bg-black/20">
                            <img 
                              src={imgSrc} 
                              className={`${isBg ? 'w-full h-full object-cover' : isEnemy ? 'w-8 h-8 object-contain' : 'w-8 h-8 object-contain'} pixel-art`} 
                              alt="" 
                              onError={(e) => { 
                                e.target.style.display = 'none'; 
                                if (e.target.nextElementSibling) e.target.nextElementSibling.style.display = 'flex'; 
                              }} 
                            />
                            <div style={{ display: 'none' }} className="items-center justify-center w-full h-full text-gray-500">
                              <FallbackIcon size={16} />
                            </div>
                          </div>
                        );
                      }
                      return <FallbackIcon size={18} className="text-gray-500" />;
                    })()}
                  </div>
                  <div className="min-w-0">
                    <div className="text-sm font-black text-white truncate group-hover:text-emerald-400 transition-colors leading-tight">
                      {data?.name || 'Unknown Entity'}
                    </div>
                    <div className="text-[10px] font-mono text-gray-600 truncate uppercase tracking-tighter">
                      {e.tag ? `TAG: ${e.tag}` : (data?.type || id)}: {id}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  {showStarToggle && (
                    <button 
                      onClick={handleStarToggle}
                      className={`p-2 rounded-lg transition-all shrink-0 ${
                        isPrimary 
                          ? 'text-amber-400 bg-amber-400/10 fill-amber-400 hover:bg-amber-400/20' 
                          : 'text-gray-600 hover:text-amber-400 hover:bg-amber-400/10 hover:fill-amber-400/20'
                      }`}
                      title={isPrimary ? "Primary Source (Propagates Cost)" : "Mark as Primary Source for Value Propagation"}
                    >
                      <Star size={14} />
                    </button>
                  )}
                  {side === 'right' && (activeType === 'task' || activeType === 'recipe' || activeType === 'enemy') && (
                    <button 
                      onClick={() => onUpdate(i, { autoBalanceChance: !e.autoBalanceChance })}
                      className={`p-2 rounded-lg transition-all shrink-0 ${
                        e.autoBalanceChance 
                          ? 'text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20' 
                          : 'text-gray-600 hover:text-emerald-400 hover:bg-emerald-500/10'
                      }`}
                      title={e.autoBalanceChance ? "Drop Chance Auto-Balanced by Calculator" : "Allow Calculator to auto-balance Drop Chance"}
                    >
                      <Percent size={14} />
                    </button>
                  )}
                  {!isTagInput && data?.type && (
                    <button 
                      onClick={() => setActiveEntity(id, data.type)}
                      className="p-2 rounded-lg bg-white/5 text-gray-500 hover:text-emerald-400 hover:bg-white/10 transition-all shrink-0"
                      title="Jump to Entity"
                    >
                      <ArrowRight size={14} />
                    </button>
                  )}
                </div>
              </div>

              {/* Controls: Qty / Chance / Locks */}
              {showInputs && (
                <div className="space-y-2">
                  <div className="grid grid-cols-2 gap-3">
                    {(side === 'left' || isQuestRewards || isLootTableEntries || isEncounterEnemies || isEncounterTableEntries || isAreaChildren) ? (
                      <div className="space-y-1 col-span-2">
                        <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest block">
                          {isQuestRequirements ? "Required Amount" : 
                           isQuestRewards ? "Reward Amount" : 
                           isLootTableEntries ? "Drop Weight" : 
                           isEncounterEnemies ? "Spawn Weight / Chance" :
                           isEncounterTableEntries ? "Drop Weight" :
                           isAreaChildren ? "Required Fragments" :
                           "Quantity"}
                        </label>
                        <input 
                          type="number" 
                          step={isEncounterEnemies ? "0.01" : "1"}
                          min="0" 
                          value={e.quantity ?? 1} 
                          onChange={(ev) => onUpdate(i, { quantity: Number(ev.target.value) })}
                          className="w-full text-xs font-bold bg-black/40 border-white/5 h-8 px-2 rounded-lg focus:border-emerald-500/50 outline-none"
                        />
                      </div>
                    ) : (
                      <>
                        <div className="space-y-1">
                          <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest flex items-center justify-between">
                            <span>Quantity (Min-Max)</span>
                          </label>
                          <div className="flex items-center gap-1">
                            <input 
                              type="number" 
                              min="1" 
                              value={e.minQty ?? e.quantity ?? 1} 
                              onChange={(ev) => onUpdate(i, { minQty: Number(ev.target.value), quantity: Number(ev.target.value) })}
                              className="w-full text-xs font-bold bg-black/40 border-white/5 h-8 px-2 rounded-lg focus:border-emerald-500/50 outline-none"
                            />
                            <span className="text-gray-600">-</span>
                            <input 
                              type="number" 
                              min="1" 
                              value={e.maxQty ?? e.quantity ?? 1} 
                              onChange={(ev) => onUpdate(i, { maxQty: Number(ev.target.value) })}
                              className="w-full text-xs font-bold bg-black/40 border-white/5 h-8 px-2 rounded-lg focus:border-emerald-500/50 outline-none"
                            />
                          </div>
                        </div>
                        {(e.dropChance !== undefined || e.chance !== undefined) && (
                          <div className="space-y-1">
                            <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest flex items-center justify-between">
                              <span>Chance %</span>
                              <button 
                                onClick={() => onUpdate(i, { isLocked: !e.isLocked })}
                                className={`p-0.5 rounded ${e.isLocked ? 'text-amber-400 bg-amber-400/10' : 'text-gray-600 hover:text-gray-400'}`}
                                title={e.isLocked ? "Drop Chance Locked" : "Solver can adjust Drop Chance"}
                              >
                                {e.isLocked ? <Lock size={10} /> : <Unlock size={10} />}
                              </button>
                            </label>
                            <input 
                              type="number" 
                              step="0.01" 
                              min="0" max="100"
                              value={e.dropChance ?? e.chance ?? 100} 
                              onChange={(ev) => onUpdate(i, { dropChance: Number(ev.target.value), chance: Number(ev.target.value) })}
                              className={`w-full text-xs font-bold bg-black/40 h-8 px-2 rounded-lg focus:border-emerald-500/50 outline-none ${
                                e.autoBalanceChance 
                                  ? 'border border-emerald-500/30 text-emerald-400 font-mono' 
                                  : 'border-white/5 text-white'
                              }`}
                              disabled={e.isLocked || e.autoBalanceChance}
                              style={{ opacity: (e.isLocked || e.autoBalanceChance) ? 0.7 : 1 }}
                              title={e.autoBalanceChance ? "Drop chance is calculated dynamically by the solver" : undefined}
                            />
                          </div>
                        )}
                      </>
                    )}
                  </div>
                </div>
              )}
 
              {/* Metrics Readout */}
              <div className="grid grid-cols-2 gap-x-3 gap-y-2 pt-2 border-t border-white/5">
                {data?.type === 'item' ? (
                  <div className="col-span-2">
                    <Metric label="PRICE" value={updates.trueCost ?? data?.trueCost} suffix="GP" color="text-emerald-400" />
                  </div>
                ) : data?.type === 'enemy' ? (
                  <>
                    <Metric label="EV" value={updates.calculatedEV ?? data?.calculatedEV} color="text-emerald-400" />
                    <Metric label="HP" value={data?.hp} color="text-red-400" />
                    <Metric label="GPH" value={updates.goldPerMinute ?? data?.goldPerMinute} suffix="/m" />
                    <Metric label="XPH" value={updates.xpPerMinute ?? data?.xpPerMinute} suffix="/m" color="text-amber-400" />
                  </>
                ) : data?.type === 'task' || data?.type === 'recipe' ? (
                  <>
                    <Metric label="EV" value={updates.calculatedEV ?? data?.calculatedEV} color="text-emerald-400" />
                    <Metric label="GPH" value={updates.goldPerMinute ?? data?.goldPerMinute} suffix="/m" />
                    <Metric label="XPH" value={updates.xpPerMinute ?? data?.xpPerMinute} suffix="/m" color="text-amber-400" />
                    <Metric label="TIME" value={data?.baseTickTime ? (data.baseTickTime / 1000).toFixed(1) : '—'} suffix="s" />
                  </>
                ) : data?.type === 'area' ? (
                  <>
                    <Metric label="FRAGMENTS" value={data?.totalFragments} />
                    <Metric label="PACK COST" value={data?.packBaseGoldCost} suffix="GP" color="text-amber-400" />
                  </>
                ) : data?.type === 'quest' ? (
                  <div className="col-span-2">
                    <Metric label="GOAL" value={data?.description || `${data?.targetEvent}: ${data?.maxProgress}`} />
                  </div>
                ) : data?.type === 'workstation' ? (
                  <>
                    <Metric label="CAP" value={data?.skillCap} suffix=" Lv" />
                    <Metric label="SUBSKILL" value={data?.subskillId} />
                  </>
                ) : data?.type === 'encounter' ? (
                  <div className="col-span-2">
                    <Metric label="ENEMIES" value={(data?.assignedEnemies || []).length} />
                  </div>
                ) : data?.type === 'encounterTable' ? (
                  <div className="col-span-2">
                    <Metric label="POOLS" value={(data?.entries || []).length} />
                  </div>
                ) : (
                  <div className="col-span-2 text-[10px] text-gray-500 italic">No diagnostics available.</div>
                )}
              </div>
 
              {side === 'left' && data?.type === 'item' && activeType !== 'quest' && (
                <div className="pt-2 border-t border-white/5 flex flex-col text-[10px]">
                  <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest block mb-0.5">Total Input Cost</span>
                  <div className="font-mono font-black text-emerald-400 flex items-center justify-between">
                    <span className="text-gray-400">{e.quantity ?? 1} × {Math.round(updates.trueCost ?? data?.trueCost ?? 0).toLocaleString()} GP</span>
                    <span>= {Math.round((e.quantity ?? 1) * (updates.trueCost ?? data?.trueCost ?? 0)).toLocaleString()} GP</span>
                  </div>
                </div>
              )}

              {/* Delete Connection Button (Unless read-only view) */}
              {showAddControls && (
                <button 
                  onClick={() => onRemove(i)}
                  className="absolute bottom-2 right-2 p-1.5 text-gray-700 hover:text-red-500 hover:bg-red-500/10 rounded-md opacity-0 group-hover:opacity-100 transition-all"
                  title="Remove Connection"
                >
                  <Trash2 size={12} />
                </button>
              )}
            </div>
          );
        })}
      </div>
    </div>
  );
}

function Metric({ label, value, suffix = '', color = 'text-gray-400' }) {
  // Round to integers for readability as requested
  const formatted = typeof value === 'number' ? Math.round(value) : (value || '—');
  return (
    <div className="flex flex-col">
      <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest select-none">{label}</span>
      <span className={`text-xs font-mono font-black truncate ${color}`}>
        {formatted}{formatted !== '—' && suffix}
      </span>
    </div>
  );
}

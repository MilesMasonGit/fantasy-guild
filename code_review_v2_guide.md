...
**This guide has drifted three times already**, in the day between being written
and being used, because deletions kept landing after each section was drafted.
Sessions 4 and 5 both had to be corrected mid-brief, and Session 5 found two
further errors on its own.

Corrections applied 2026-08-19: the registry count (23 → **20**),
`areaSetRegistry` (deleted, no ruling needed), `questConfig.js` (deleted, not a
starting point), and `nameRegistry.js` (**live via the barrel — do not delete**).

**The rule this establishes: verify a territory list against the tree before you
start, and report drift rather than working around it.** A plan that names
deleted files is the exact failure the re-scope existed to prevent, and it
recurs whenever the codebase moves faster than the document. Sessions 1–5 each
also corrected at least one *ticket* that turned out wrong or moot — treat both
the plan and the findings as evidence to check, not as truth to inherit.

...
| **Pass 2 — The Tree** | | |
| 4 — The job tree as data | ✅ Done (2026-08-12, `skill-class-rework`) | **Tests 678 → 836 across 48 files, all green**; build clean. `jobRegistry.js` holds all 19 entries. ⚠️ **Each job declares its complete SHEET, not its deltas** — what a promotion grants and removes is derived by diffing against the parent, so a sheet cannot drift out of agreement with its own parent and **re-parenting a job recomputes its deltas automatically**. Everything else derives from each skill's `layer`: which skill is the combat one, which is the signature, which are foundation. Nothing outside a `skills` array names a skill. `HeroGenerator` now reads the job sheet rather than the Foundation list, so changing what a Recruit holds is a one-file edit. **`JobTree.test.js` (137 tests)** asserts every structural rule — sheets exactly 6, foundation pairs a subset of the parent's, combat and parent-shared carried forward, signatures unique and all 12 granted, coverage even at 4 apiece, tree connected, gates never removed by the promotion that checks them. ⚠️ **Mutation-checked, not just written green**: reverting one of D-268's swaps failed exactly the two coverage rules it should, so the suite is not vacuous. `classRegistry.js` survives for sprites only — Phase 7/9 retires it with `traitRegistry`. |
| 5 — Promotion, re-training & banking | ✅ Done (2026-08-12, `skill-class-rework`) | **Tests 836 → 858 across 49 files, all green**; build clean. `PromotionSystem.js`: `canPromote` / `promote` / `previewPromotion` / `getAvailablePromotions`. **Promotion and re-training are one operation** (D-248) — re-training is not an undo, it is entering a job at the same price, so "forward" and "back" are not two systems with two rule sets. **Banking works both ways** (D-71): a removed skill goes to `hero.bankedSkills` at its level and returns from there intact, and **banked skills count toward a later gate** — a hero who reached Cooking 30 has not forgotten how to cook, so re-training does not make them earn it twice. Costs are taken only after every check passes, mirroring the Cartographer. ⚠️ **NOT browser-verified, and here is why**: promotion has no screen until Phase 8, and Option 1's route is blocked — git refuses to check `skill-class-rework` out in the main folder while the worktree holds it, and `preview_start` stays pinned to the launch directory. Its one visible effect (the Dock's six cells changing contents) is covered instead by three tests over the actual UI contract: `heroes_updated` fires, `hero_promoted` carries the whole trade, and the projection `DockSkillsGrid` computes changes to the new sheet. ⚠️ A test bug worth remembering: the first draft faked a restore by writing a banked skill back into the held sheet, which hid the very thing under test. |
| 6 — Roster, recruitment & the Market shift | ✅ Done (2026-08-12, `skill-class-rework`) | **Tests 858 → 870 across 50 files, all green**; build clean. Roster cap **10 → 12** (`roster_size` `maxRank` 5→7, D-251) with a test on the *join* between the definition and `GuildUpgradeManager.recompute`, since those numbers live in different files. **The class/trait reveal is gone from recruitment**: every candidate is now mechanically identical (same six skills at level 1), so advertising a rolled attribute implied a difference that does not exist — the card states the job and the skills they arrive with instead. Markets were already re-keyed to `commerce` in Phase 1; this phase **asserts** it — every Market wants Commerce, a non-Merchant raises `UNSKILLED` and earns no gold, **Merchant is the only one of the twelve jobs that brings it**, and raw selling still works with no hero, no Token and no skill. ⚠️ **Not visually verified: the recruitment candidate card.** The Guild Hall screen opens from the centre board tile and I could not drive that reliably from the console; the data contract and a clean module load are covered, but the card's new layout gets eyes in Phase 7/8, which are the UI passes anyway. |
| **Pass 3 — Presentation** | | |
| 7 — The hero card & inspection modal | ✅ Done (2026-08-12, `skill-class-rework`) | **Tests 870 across 50 files, all green**; build clean. New `HeroSkillSheet.jsx`, mounted at the top of `HeroEditModal`: job, lineage (`Recruit → Fighter`), the six held skills **grouped by layer** (derived from the registry's own layer order, so re-layering a skill needs no edit here), and a **"Set aside"** block for banked skills — greyed, dashed, with the promise spelled out: *"Kept at the level they reached. A job that uses one again gets it back exactly as it is."* ⚠️ **This is the first time reversibility is visible to the player rather than a rule in a document.** Verified live on a promoted Fighter: modal reads Mining/Logging/Smithing/Crafting 14, Melee 1, Leadership 1, with Fishing 23 and Cooking 9 set aside; the dock card shows those same six and **not** the banked pair — both halves of D-250 confirmed. Empty bank correctly renders no section at all. ⚠️ **The roadmap's clutter worry was based on a wrong assumption and is void**: "12 heroes × 6 skills = 72 values on screen" cannot happen, because skill cells render only on a *pinned* card and at most two pin at once. Measured at a full 12-hero roster: **12 skill cells and 66 visible text elements**, against 47 with nothing pinned. |
| 8 — The promotion & re-training screen | ✅ Done (2026-08-12, `skill-class-rework`) | **Tests 870 across 50 files, all green**; build clean. New `JobChangeModal.jsx`, opened by a **Change job** button on the hero sheet. **One screen for promotion and re-training** (D-248) — every reachable job listed the same way at the same price, whether it is a step down the tree, sideways between siblings, or back to something held before; there is no "undo" affordance, because reversal is not a correction, it is just another job. **Ineligible jobs are shown, not hidden**, each with its exact shortfall (*"Needs Mining 12/25, Smithing 12/25, Melee 0/25, Leadership 0/25"*) — a job that silently vanishes teaches nothing. **The trade is stated before the click**, never after. Driven end-to-end in the browser: Recruit → Fighter gave up Fishing 21 and Cooking 6, gained Melee and Leadership, spent exactly 250g and 5 copper ingots; then re-training Fighter → Rogue showed **"+ Fishing 21 — back from set aside"** and restored it at 21, banking Smithing 12, Melee 1 and Leadership 1. ⚠️ **Copy bug caught by that run**: the intro read "swaps two skills for two others", which is true down the tree but false across branches — a lateral move swaps three, since the combat skill and shared specialist change too. Reworded to something true in every case. |

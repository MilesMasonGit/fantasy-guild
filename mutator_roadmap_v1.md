...
| 10 | Content catalog & counters | 🟡 Partly done | `e1d0389` + `PENDING` | **§14 token catalog authored** (abundance / trawler / hex / cursed / dam) plus four mutator ACTION cards (`data/cards/action/mutators.json`) and a `MUTATOR` preset. **Acquisition wired and VERIFIED in-game:** `binderCatalog` now admits `action` (whitelist + an "Action" tab), and the Guild Hall `deckList` offers the four mutators, so they drop from the unified pack pool. Confirmed by pulling *Bountiful Rites* from a pack — it renders with its `ACTION · NATURE · GATHERING` tags, appears in the Cards drawer as available, and slots into a deck via Inspect → Add to Deck. **REMAINING:** (a) the reusable-vs-consumed split (§15.7); (b) a Mutator in the LAST deck slot silently stamps nothing — forward-only targeting means it has no upcoming slots, which needs a UX warning or an authoring rule; (c) a token badge has still not been *seen* rendering — see the note below. |

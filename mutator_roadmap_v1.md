...
| 10 | Content catalog & counters | 🟡 Partly done | `e1d0389` | **§14 token catalog authored** (abundance / trawler / hex / cursed / dam) ahead of Phase 9, since badges can't be seen without tokens. Four mutator ACTION cards authored (`data/cards/action/mutators.json`) with a new `MUTATOR` preset; `cardRegistry` folder map now knows `action`. **REMAINING: mutator cards are not obtainable or slottable in-game.** The Cards drawer's SHOW filter doesn't recognise `action`, and nothing grants ownership, so a player (or a verifier) cannot put one in a deck. That plus the reusable-vs-consumed split (§15.7) and Dam's counter content is what's left. |

...
| 6 — Combat on the Board | ✅ Done (2026-08-06, `playmat-7x7-build`) | **D-136 held: a port, not a rebuild.** The 7-stat engine is untouched — `BoardCombat.js` is a thin adapter holding one ephemeral card-shaped object per fighting tile, the same bridge the deck loop used between flyweight slots and the execution engines. **Combat has a tick owner again**, closing the gap analysis §2.2 hole. **Tests 461 → 476 across 34 files** (+15 `BoardCombat`), build clean. Verified live: an enemy Token with no hero did **nothing** across 5s of ticks (D-14, inert until targeted); placing a hero started a fight (enemy 12/33, hero 100→50); **retreat by unassigning reset the enemy to full HP** (`G-4`) so a boss cannot be chipped down across free attempts; a capable hero killed 9 times, spending 9 charges (D-104) with loot landing **as sprites on the board** rather than in the Bank; a Sawmill adjacent to the enemy wore 10→6, proving **one kill is one cycle** for wear (D-129/D-126); a weak hero was **wounded, removed from the board, and the tile left free for someone else** (D-74). ⚠️ **Two bugs found by the tests, both real**: `servesFrom` checked only for `config`, which silently exempted combat from support wear because enemy Tokens carry `enemyId` instead; and the fight-end hook keyed off `HERO_MOVED`, which fires with `tile: null` on a recall — it names where the hero *went*, not where they came from — so it never fired and a damaged enemy would have persisted across retreats. Both fixed. ⚠️ **Probe note**: tiles 20 and 21 are **not** adjacent (they wrap a row edge), which briefly looked like a wear bug — the same trap `Adjacency.test.js` guards against. **Minor pre-existing issue surfaced by stress-testing**: rapid kills interrupt the victory SFX (`play() request was interrupted by pause()`); harmless at real pacing, out of scope here. |
| 7 — Banks, Managers & Guild Upgrades | ✅ Done (2026-08-06, `playmat-7x7-build`) | **The AFK story is real: gold → Vault → Manager → board.** New: `Managers.js`, `TokenBank.js` (consolidation + slot cap + selling), `TokenVaultTab.jsx`, `board.vacancies`. **Tests 476 → 525 across 37 files** (+49: Managers 17, TokenBank 20, Consolidation 10, +2 in TokenCycle), build and console clean. ⚠️ **The phase was blocked on something §7 does not mention:** a hero's position lived on the Token instance, so **a hero could not outlive the Token they stood on** — a depleted Forest deleted the instance and took the person with it, back to the Dock. D-151 is unbuildable on that model, and D-57 and D-60 were quietly unimplemented too. Hero position moved to `board.heroTiles` as its own state (owner decision 2026-08-06); the knock-on effects are all the design's stated intent — a hero stands on a bare tile as placed-but-idle, moving or lifting a Token leaves its hero standing there, and a Token dropped *under* a standing hero is worked without re-placing them. Verified live: a Forest on its last charge vanished, a Lumber Camp pulled a fresh one from the Vault, and **the hero kept producing without being touched** (4999 uses a moment later); with the Vault empty the tile stayed bare, carried a red mark reading *"This tile ran dry and the Vault has no replacement — restock it"* and the hero carried the yellow one — **risk 15 checked, "I ran out of stock" is distinguishable from "something else went wrong"**; depositing a Forest restarted it unattended within one sweep. Consolidation matched the roadmap's worked example exactly (3,000 + 4,000 → one full 5,000 + a 2,000 remainder), and a 20× pick-up-and-replace loop conserved the total to the charge. A second Heartwood was refused with *"Only one Heartwood can be on the board at a time"* while the Vault happily held five (D-177); Mythics sell at 120g with **no block** — the roadmap's ⚠️ rested entirely on "one copy ever", which D-177 struck. Both Guild tracks bought and applied (Vault 12→16 slots, roster 5→6), and the Guild Hall **tile** opens the tree. With Tray and Vault both full a Token stayed on the floor rather than being destroyed (D-138). ⚠️ **Fourth environment trap**: `requestAnimationFrame` never fires in a non-compositing tab and the nav bubbles defer by one frame, so **every bubble looks dead** under automated verification — shim rAF inline before concluding anything is broken. ⚠️ **Pre-existing, not fixed**: `isHeroIdle` reports a hero in combat as idle (enemy Tokens carry `enemyId`, not `config`); only tests read it today. |
| 8 — The Cartographer & Maps | ✅ Done (2026-08-07, `playmat-7x7-build`) | **Progression, and the reward beat.** New: `mapRegistry.js` (two placeholder Maps — Woodland 200g, River 2,000g), `Cartographer.js`, `CartographerTab.jsx` (a drawer pane, owner decision), Map Tokens (`uses: 1`, no rarity), and **Market Tokens (D-141), which this roadmap does not schedule at any phase** — added at the owner's request because Maps are the primary gold *sink* against a faucet that was otherwise just selling loot at `baseValue`. **Tests 525 → 561 across 40 files** (+36: Cartographer 16, MapBurst 13, Market 7), build and console clean. ⚠️ **Two placeholder Maps, not one, deliberately**: D-166's curve is *flat within a theme, stepped between them*, and one Map cannot prove the step half at all. Verified live: the shop listed both Maps in price order with the River Map stating *"Not enough gold — 2000g needed"* rather than greying out (D-99); **24 silhouettes on a fresh save**, the complete pools of both Maps; buying took 200g **and pulled 5 Oak Wood from the Bank** (D-150) and put the Map in the **Tray with the Vault untouched** (D-156); **four consecutive purchases all cost exactly 200** (D-166's load-bearing half); a full Tray refused the purchase with a readable reason and **spent nothing** (D-160); a double-click in the Tray burst the Map into 4–5 scattered sprites and consumed it (D-142/D-155); and the three things that came out **filled in their silhouettes**, taking the shop from 24 unknowns to 21 with *Forest*, *Tool Rack* and *Oak Wood ×12* now named — D-159's restocking-guidance loop closed end to end. A Lumber Market with a hero on it turned 10 Oak Wood into **34 gold with no sprite dropped**, closing board → goods → gold → Map on the board itself. ⚠️ **The one thing not verified is the thing the phase is for**: D-167 reserves "did the burst feel like a reward or a chore" for the owner, and that judgement has not been made — the mechanics are pinned, the feel is not. **Doc correction:** D-170's ⚠️ *"needs answering before loot tables are authored"* is stale — **D-175 already struck and answered it** (Uncommon needs no defining attribute). D-169 and D-170 now carry strikethrough markers like D-50; both had read as live. |
| 9 — Content: Map 1 and Map 2 | ✅ Done (2026-08-07, `playmat-7x7-build`) | **Two authored kits, and a new game that opens playable.** Woodland (23 Tokens) and Riverlands (8). New: `ContentRules.test.js` (45), the D-122/D-123 starting state, tool-gating live. **Tests 561 → 606 across 41 files**, build clean, no broken images on a fresh game. **The arc, played from a genuinely new save:** 1 hero (*Hammer*, Lv1), 120 gold, four Tokens in the Tray, empty board, Cartographer already open. All three starting producers are workable at level 1 — placing them and stationing the hero took well under a minute, so **the core loop is reachable in the first minute**. One staffed Oakwood Grove produced **12 Oak Wood in 60s across 6 cycles** (24 nature XP, Sawmill's +5% applying), which is **24 g/min** — so 120g → the 200g first Map is **~3.3 minutes**, and the progression loop is comfortably a first-session event. Bought, burst, and got 2 Bramble Patches + a Copper Seam + 8 Copper Ore. ⚠️ **Tool-gating verified live and it behaves exactly as D-213 intends**: a Yew Stand with no axe shows **`no_recipe`** and makes nothing; an axe beside it clears the alert, yields 3 Yew Logs, and **the axe wears 80 → 79**. That wear is what makes deadlock real, which is what rule 1 guards. Access fires *before* the recipe check (a Lv1 hero sees "skill too low", not "no recipe"), which is the right priority. Riverlands' skill wall confirmed: a Lv1 hero on a River Delta raises `access`. ⚠️ **Risk 17, reported as asked: 23 Tokens, not ~15 — and the pain was not the numbers.** Tuning is quick; what cost the time is that **engine tests pin content values** (`TokenCycle`, `AdjacencyEffects` assert specific yields and item ids), so retuning content breaks tests that are not about content. **Decouple engine fixtures from shipped content before 60 Tokens** — at 23 it cost an hour, at 60 it will be why nobody retunes anything. ⚠️ **Only 4 enemies are usable**: `enemyRegistry.js` defines 18 but every one drops legacy item ids absent from `data/items.json`, so a kill silently resolves to no loot; only `data/enemies.json`'s four point at real `item_*` ids. Enemy variety is capped at four until the duplicate registries are resolved. **Bug found and fixed:** two Tokens named sprite files that do not exist — invisible Token, a 500, and no other symptom; a content rule now asserts every sprite is real. |
| 10 — Polish, Clutter & the First Balance Pass | ✅ Done (2026-08-07, `playmat-7x7-build`) | **The rework is complete.** Deliverable is [`playmat_balance_report_v1.md`](playmat_balance_report_v1.md), a written report rather than a code summary. **Tests 606 → 613 across 42 files**, build clean, tagged **v0.5.0**. ✅ **Risk 7 closed**: on a worst-case 48-Token / 8-hero board with both alert colours, work in progress and loot down, **17 elements compete with the art, 64 total** — against 128 at Phase 3 and 84 at Phase 4, so **the board got quieter as it gained systems**. The ring is mounted on 48 tiles and *drawn on 4*; alert marks appeared on exactly the 2 tiles with real problems (D-149 refusing to flag unstaffed Tokens is doing the heavy lifting). ⚠️ **Risk 13 measured, not guessed, and it is milder than feared**: under sustained shortage the deep consumer (needs 5) took **48% of the scarce material** vs the shallow one (needs 2). The feared failure — deep chains squeezed toward zero — does not occur; allocation splits supply almost evenly *by volume* and the deep chain converts its half into fewer, larger outputs. **The completion count (8 vs 3) is the misleading number, and it is the one the risk was framed around.** No change to D-127; a guard test pins input share above 30%. **Pacing on target**: 85 completions / 300 board-seconds with 7 heroes = 3.53s each, ~3.1s at a full 8-hero roster against D-164's 2–3s intent. **Bug found: D-145 was never implemented** — the inspection panel handled items only and its own comment claimed a Token branch was "added in Phase 4"; the Vault's and Cartographer's inspect clicks silently did nothing. Token detail now works from Vault, Tray, Cartographer pool and board, carrying planning-before-placement (D-145), **visible pairings** (D-18) and where-to-restock (D-159) in one sheet. ⚠️ **Three questions stay open and are the owner's to judge**: does a burst feel like a reward (D-167), is a board with no antagonist interesting for an evening (risk 5), does adjacency read as meaningful (risk 2). The report states them rather than answering them. **Not done:** audio for placement/burst/depletion (§E) — additive, no systems work needed. |

| 10R-1 — Refinement: Tokens as physical objects | ✅ Done (2026-08-07, `token-object`, uncommitted) | **Sits inside Phase 10** as R-1 of [`playmat_refinement_briefs.md`](playmat_refinement_briefs.md). Intent spec [`token_object_intent.md`](token_object_intent.md) (**D-215…D-222**) approved before any code. One `<TokenSprite>` behind all call sites; Token art at **two sizes only — 128px in play, 64px in storage**, Cartographer chips 32px as a recorded exception; **no frame on any surface**; bloom retired for a shadow-and-offset lift; the loot ring replaced by **items hover, Tokens rest**. **Tests 613 across 42 files, unchanged.** ⚠️ **Two corrections to the brief, both found by re-verifying and both material**: it counted *six* components at *five* sizes — it is **eight at six**, because `CartographerTab.jsx` (22px) and `TokenInspection.jsx` (48px) were missed, which is the case for the shared component stated better than the brief stated it; and **Token source art is 64×64, not 32×32** (the 32 was measured from the placeholder *skill* icons; `.agent/skills/Artist/SKILL.md` specifies 32=Items/64=Tokens and 15 finished sprites already exist). That **amends D-171** — `ART_PX` 32→64, `TILE_SCALE` 4→2, `TILE_PX` and `BOARD_PX` **identical either way** — and means the board tile's 96px was itself fractional (1.5×), so **the board would have rendered blurry the moment real art was wired in**. Verified live: every `/assets/` image in the running DOM measured 128 or 64 with no other Token size present; Tray slot borders `0px`; zero `ring-gi-primary` elements on the floor; items carry the hover animation and detached shadow while Tokens carry the tight resting one; a Token dragged Tray→tile carried at **128px with no frame, scale 1, rotation 0**, and dropping it moved the board 4→5 tiles and the Tray 5→4. **Bug I introduced and caught:** the ghost first rendered at **74px** — dnd-kit sizes its DragOverlay to the source node and Tailwind's preflight `img { max-width: 100% }` then clamped the art to it, which is bloom by accident; fixed with `max-width: none` on the sprite plus an explicit box on the ghost. **Owner decision during verification:** the shared drag wrapper's `scale: 0.72→1` spring and `-4°` rotation were stripped for **every** ghost kind, heroes and items included, rather than confining the fix to Tokens. ⚠️ **Handed on, not solved:** full-bleed tile art puts the alert dot, progress ring and hero chip **on artwork** — **R-7** and **R-6** own that legibility, and R-1 deliberately did not grow D-85's mark budget to compensate. **Still unwired:** the 15 real 64×64 Token sprites in `public/assets/tokens/`; out of scope by the brief, so this work is *sized* for them and *verified* with placeholders. |

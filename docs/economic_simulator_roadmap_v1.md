...
kickoff brief's named first task). **P2 → P2.5 → P3+4 → P5 → P6 → P7 → P8 →
P9** is strict (P3 and P4 merged by owner ruling 2026-08-28). P1 must land before P7 (the Map check reads the burst constants
live).
...
| **P1** | Burst rules in the game | Exactly 3, first slot always a Token (CMS-129) | — | **Done 2026-08-28.** `BURST_MIN`/`BURST_MAX` replaced by one exported `BURST_SIZE = 3` (`Cartographer.js:67`); `rollOne` generalised to take an entry subset so slot one's Token-only draw renormalises through the existing arithmetic rather than a second loop. Guild Hall branch byte-identical, commented exempt by ruling 24 and pinned by a test. Verified in the running game: 10 real buy-and-open runs, 3 things and a Token-led slot one every time; Guild Hall still single scripted drops in sequence. `BoardRunner.js`'s "a gate, and only a gate" doctrine comment corrected (S10) — Speed is live, Efficiency is the real hole. Three stale "3–6 items per burst" sites in the decisions log annotated. Tests 1381 → 1388. ⚠️ **The guarantee is not absolute:** a Token entry authored at `weight: 0` is undrawable, so such a pool degrades to three free draws with no Token. Unfixable at the roller — **P7's Map check must warn on zero-weight Token entries as well as on token-less pools.** Documented in code and pinned by a test. **Observed, no action needed** (owner, 2026-08-28: "the pools will grow and change throughout development — that's why we need a simulator that can run and recalculate as development proceeds"; roadmap §1.5): 5 of the 6 purchasable shipped Maps have 1- or 2-entry pools (`map_oak_forest`, `map_bronze_hills`, `map_cozy_hamlet` are single-entry and now yield the same Token ×3 deterministically); every shipped pool entry is `kind: "token"`, so slot-one filtering is a **no-op on all shipped content** and renormalisation ships dormant. The live change for players is purely 3–5 → 3, i.e. a ~25% yield cut on the three single-entry Maps. Recorded as a fact about today's substrate, **not** as a content gate on any later phase. |
| **P2** | Vocabulary + intent schema | Tags authorable; `epic`; tempo bands; intent seeded; 30s rule relaxed | — | **Done 2026-08-28.** `epic` in `TOKEN_RARITIES` between rare and mythic (interim `SELL_VALUE.epic = 70`, an implementation default superseded by P7). New `src/config/registries/tempoBands.js`. `ContentRules` Rule 4 split tagged/untagged. CMS: one shared `SimIntentControls` on both editors, `downcycle` on recipes, per-output `baseQty`/`variable`/`anchor`, intent seeded mechanically. Verified in the CMS against a genuine versionless workspace. Tests 1388 → 1441. ⚠️ **Owner ruling — band scaling is `1 + (level - 1)/70`, NOT the plan's literal `1 + level/70`.** §13.3's formula and its own printed level-1 column disagree by 1.4%; the printed table won, so level 1 is exact and a round authored cycle time lands in band. Worst fit against the level-40/90 columns loosens 0.57s → 1.43s. ⚠️ **zustand never calls `migrate` on a versionless blob** (`typeof version === 'number'` guard, `middleware.mjs:392`) — every workspace in existence today has no version, so the seeding hangs on **`merge`**. **P5 must do the same**: a workspace opened and never edited stays versionless, so a `version: 2` migration would skip it too. `useGlobalStore.js:75-77` has the same dead `version === undefined` branch. ⚠️ **Found, not fixed — fix before P5:** `CMSBoundary.test.js`'s scanner regex lets an `export const` with no `from` run forward to the next `from`, producing nonsense failures naming the wrong file. P2 works around it by line placement in `cms/src/utils/constants.js`, which is fragile, and P5 adds and removes several cross-boundary imports. |
| **P2.5** | Tag the shipped corpus | ~~**Owner sitting**~~ **Delegated to the implementer 2026-08-28** — see status | P2 | **Done 2026-08-28.** ⚠️ **No longer an owner sitting.** The owner delegated it: *"Your job is to implement the system, and I'll be able to use the system to make the game work how I want afterwards… I'm fine with you going in and tagging them however you want. This is all early development, I'll change them as needed."* **Every tag is therefore provisional test substrate, not design** — chosen to give the sim a spread it can be judged against. Nothing downstream may treat them as authored intent. 16 cycled Tokens (not S21's 15 — `token_shrimp_market` is a market kind that carries a work cycle) + 3 recipes tagged; 0 of the 23 inert Tokens tagged. Spread: 6 fast / 6 medium / 3 slow / 1 heavy, 10 iph / 4 gph / 2 xph. ⚠️ **Cycle times were re-timed to match tags** (10 of 19 entities) — every shipped Token was level 1 at 10–12s, so only Fast and Medium were reachable and Slow/Heavy would have gone unexercised. Each re-timed entity went to the middle of its band, which is what P3's `tempoPass` will derive anyway. No test pinned a shipped cycle time. **3 anchor flags**, each reasoned: `token_oak_tree`→oak wood and `token_copper_ore_vein`→copper ore break all-key ties between identical level-1 commons; **`recipe_charcoal`→charcoal overrides a default that would elect wrongly** — `token_campfire` is level 1 and would win lowest-level, but it is **mythic**, and pricing a common material off a mythic producer is the exact failure the override exists for. **2 `variable` outputs introduced** (shrimp coast 65%, quartz deposit 50%) so P4 exercises that path both inside a chain and at a leaf; ⚠️ this does change gameplay — raw shrimp now arrives ~35% less often. ⚠️ **`ContentRules` Rule 4's untagged branch is now empty** and gained the same empty-list guard the tagged branch had (`it.each([])` throws). Kept rather than deleted — it is the rule a later untagged Token still has to meet. ⚠️ **`cms/backups/` is gitignored**, which the roadmap's "commit a workspace export" instruction did not know. The tagged workspace `cms-workspace-2026-08-28-p25-tagged.json` is re-included by name in `.gitignore` so it is tracked while staying where the CMS's File Manager can serve it. **P5's first Sync must run from a store loaded with it**, or the tags in `data/` are overwritten. |
| **P3+4** | **Engine: time, anchors, pricing** (merged) | Passes 1–3 as one tested unit, engine-only, nothing wired | P2.5 | **Done 2026-08-28.** `cms/src/engine/sim/`: `fieldAdapter` (one normaliser at the edge; `liveCharges` reads `uses`, never `charges`), `dials`, `rows`, `tempoPass`, `anchorPass`, `pricingPass`, `simRunner`, plus `dryRun.mjs` standing in for the dropped preview. Pure — verified by deep-freezing a token and running the corpus through. Unwired: `recalculateEconomy` still calls `runFullBalance`, and nothing outside `sim/` and the tests imports it. No iteration anywhere: the only unbounded loop is Kahn's algorithm, exiting when a sweep prices nothing. Tests 1441 → 1501, no existing test changed. Dry run byte-identical across runs; 12 of 18 items priced, 6 orphan Criticals (genuinely producerless content), 1 residual Warning. ⚠️ **Defect found in verification and fixed: multi-output pricing was order-dependent on entity id.** A co-output the entity does *not* anchor is subtracted from its target, so it is a dependency edge — but the walk's readiness test waited on inputs alone, and sorted-id order decided whether the co-output was priced first. Two economically identical corpora priced differently depending on what the entities were **named**. Invisible on shipped content (no shipped producer mixes anchored and non-anchored outputs) and idempotence held throughout, because the wrong ordering was a *stable* wrong ordering. Closed by `coOutputsSettled`, widening the Kahn edge set without introducing iteration; pinned by a rename-equivalence test that fails without the fix. ⚠️ **Also fixed:** a mistyped Purpose was silently worth 0 g/hr (Tempo already named its typos) — now an `unknown-purpose` Info row. **Seven judgement calls** where the plan is silent were flagged and audited; all stand. The load-bearing one: the multi-output split **compounds twice**, so an output 10× scarcer is worth **100×** per unit — verified against the prior art the plan cites (`anchorCalculator`: weight `1/yield`, then `value = target × weight / yield`) and pinned end-to-end. **Stickiness is structurally unreachable until P5 writes `valueSource`** (`grep -c valueSource data/items.json` = 0), so that path stays fixture-proven through the cutover. ⚠️ **P3 and P4 MERGED, owner ruling 2026-08-28.** The owner asked why the button could not simply run the whole simulation, and ruled the two engine-only sittings into one. **P3's read-only CMS preview surface is dropped** — it existed so a developer could review the anchor elections before anything wrote, and this owner has explicitly delegated that judgement (*"Your job is to implement the system, and I'll be able to use the system to make the game work how I want afterwards"*). The passes themselves are unchanged and are still the real engine, not a prototype. **The P4/P5 split is deliberately kept**: the arithmetic lands fully tested before the cutover rewires anything, so a wrong number after P5 is unambiguously the wiring. |
| ~~**P4**~~ | ~~Pricing pass, engine-only~~ | **Merged into P3+4** (owner ruling 2026-08-28) — see the row above | — | **Closed 2026-08-28 without its own sitting.** |
| **P5** | The cutover | Recalculate runs the new engine; old engine deleted; EV fields die; items sell right | P3+4 | **Done 2026-09-01.** Both halves in one sitting — the split point was not needed. `recalculateEconomy` runs `runSim` + the new `sim/writeBack.js`; all ten old-engine modules and `CMSBalanceEngine.test.js` deleted; `useGlobalStore` at version 2 with the §14 dials; the recipe bypass retired. Downstream contract intact — token types, `composeTokenDescription` (3 args), `auditConnectivity`, `setAuditResults` (7 args); the auditor is untouched and takes sim rows as one line of prose each. Data migrated: 12 of 18 items priced, `valueSource` on all 18, twelve retired field names gone from `items.json`/`tokenRecipes.json`. **`CommerceSystem` reads the derived `value`** — the every-item-sells-for-1g bug (S4) is dead; copper ingot 1g → 12g. Tests 1501 → 1528 (28 skipped: `CMSBalanceEngine`'s 3 went with it). CMS-134 verified across all 39 descriptions. ⚠️ **`isPrimarySource` → `anchor` is migrated BEFORE the passes run**, not after — migrating afterwards would price the same content two different ways on consecutive runs. ⚠️ **Deviation:** plan §16's *"every item has a `valueSource`"* is implemented as a weaker assertion — the field is present on all 18 and names a real source wherever a value was derived — because six shipped items have no producer at all and the strict form cannot pass. The roadmap's §6 already anticipated needing this scoping; the test records where to tighten it. ⚠️ **`taskSolver.js` no longer loads** — it imports the deleted `evCalculator` and `mockBattle`. Nothing imports it, and its header now says so plainly. **P6 must port its chance-snapping and delete it, not repair it.** ⚠️ **Found, pre-existing, NOT a regression:** `connectivityAuditor` reads `token.outputs`, but outputs live at `token.config.outputs` — verified identical before and after this phase — so it reports nearly every produced item as unreachable and the audit panel is largely noise. Its own slice, not P5's. |
| **P6** | Lever policy + refusals | Pass 4, audit cards, churn report | P5 | **Done 2026-09-01.** `sim/tuningPass.js` implements the plan's lever policy verbatim - band judged on the source's **total** profit/hour (F2), levers in order (quantity range with spread preserved, then chance where the author made it variable snapped 10/5/1, then cycle time inside the band, then refuse), one lever at a time from an untouched baseline, over-3x refuses outright except an IPH quantity move which travels freely and files a Warning past 3x (2026-08-28 ruling). `sim/refusals.js` carries the refusal catalogue as cards; the anchor and pricing passes take severity and remedies from it. `sim/churn.js` + `sim/answers.js`. CMS: `SimAnswer.jsx` on both editors (tuning diff, earn gauge, inline refusal cards, stale badge), churn report in the panel. **`taskSolver.js` deleted** after porting its chance-snapping. Tests 1551 to 1602. **The legibility test is mechanical now:** a test asserts no remedy contains a bare number, so every card names a tag or a dial. Verified end to end - tagging a refused Token slower cleared its refusal, churn reported 1 cleared. ⚠️ **Deviation, accepted:** the plan's half-unit EV steps are unreachable. The runtime rolls `min + floor(rand x (max - min + 1))`, so endpoints must be integers; integer endpoints + preserved spread + half-unit EV are mutually incompatible. Integer endpoints and the authored spread were kept; an odd spread still lands EV on a half-unit, which is the relief the plan was reaching for. ⚠️ **New field `baseChance`** - the schema lists `chance` as derived with no intent counterpart, so tuning it would ratchet over the author's number and the diff would vanish on the second run. Mirrors `baseQty`. ⚠️ **`unclosed-residual` Warning demoted to `integer-residual` Info** - the lever policy judges total earnings and speaks for itself; a Warning there filed two rows for one situation. ⚠️ **Fixed in passing** (latent gap flagged by the P3+4 verification, hit by real content): an output that can **never drop** (0-0 range, or 0% chance) was still allowed to anchor its item, pricing it off a supply that does not exist. Such outputs are no longer sources, so the item falls through to the existing orphan Critical. ⚠️ **The yield pin was re-founded.** It compared the simulator's average quantity with the game's on the same shipped output; after P6 a tuned output's derived values differ from its intent **by design**, so that comparison would fail on exactly the outputs the sim did its job on. It now pins the arithmetic (both sides agree given the same numbers) plus a stronger alarm: **every divergence between intent and derived must be explained by a recorded tuning move**. Verified to bite. ⚠️ **Found, not fixed:** `AuditPanel` had never been routed - written for CMS-74 and never mounted, so the economy audit has been invisible since it was built. P6 added an Economy Audit tab because the churn report needed the Dashboard zone. `connectivityAuditor`'s `token.outputs` bug still makes most of its rows noise. |
| **P7** | Map economics | Pass 5, derived weights, scrap, two-sided check, Map table | P5, P1 | **Done 2026-09-01.** `sim/mapPass.js`: pool weights derived from the rarity table (CMS-124), scrap allocated **aggregate-first** and summing to the budget exactly whatever the pool length or premium (CMS-48 - pinned by a test across both), and the two-sided check reading `BURST_SIZE` **live** from `Cartographer.js` with slot-one Token renormalisation modelled. Guild-hall Maps skipped (ruling 24). Map level derived as the pool-share-weighted mean. Seven new refusal cards; `MapEditor`'s weight column read-only-derived; a **Map Economics** tab beside P6's churn report; Map dials in `SettingsModal`. `TokenBank.sellValue` prefers a derived `scrapValue`, falling back to the rarity table - proven live in the running game (5 to 137 to 5 as a scrapValue was injected and removed). Tests 1602 to 1655. ✅ **Enemy valuation: one kill = one charge** (owner, 2026-09-01). Enemies carry no charge field of any kind and none exists as a Token; a kill consumes the enemy, and D-129 already counts a kill as one cycle for every board system outside combat. Lifetime loot is therefore the expected drops of one kill. ⚠️ **A real bug caught in the builder's own first draft, now pinned:** `earningsPerHour` reads a **precomputed** `abundance` per output, so a hand-built output object earned `NaN` an hour - and `NaN < bound` is `false`, which made a Map producing nothing read as **PASSING** its productive bound. Guarded to zero and covered by a test, because a verdict wrong in the reassuring direction is the worst kind. ⚠️ **Implementation defaults, owner's to overrule:** return pins interpolate `early` at level 1 to `late` at level 99, straight-line (the plan says "over Map level" without naming endpoints); `mapScrapRatio` also accepts an `{early, late}` pair, defaulting to the plan's single 0.40; gold entries count at face value on both sides outside the allocation, extending F10's raw-item rule to the kind the plan never named; the enemy band is 3x, reused from the lever policy's cap; a Token in several pools takes the **highest** slice it earns anywhere; and a non-support Token with no solved cycle counts as producing nothing with an Info row, rather than being made neutral, so an underwater Map cannot hide behind its own dead entries. ⚠️ **Fixture-proven only:** enemy, gold and raw-item entries. No shipped pool contains one, the Map editor offers `token`/`item` only, and the CMS store still loads no enemies - the read path exists and is threaded, but `state.enemies` is always empty. Stated in the test header per precedent. ⚠️ **Content findings for the owner, not bugs:** three Maps read scrap-rich purely because their pools are one or two entries long, so three draws hand over three full slices of a budget meant to cover the whole pool; Bronze Hills and Cozy Hamlet are underwater because their entire pool is a support Token, correctly counted neutral, so nothing in them produces; one Token in Oak Forest solves at ~92,000g/h and carries that Map's pass single-handed; and Test Map at 1g has a scrap budget that rounds to zero. ⚠️ **Found, not fixed:** `BURST_SIZE` is imported straight from `Cartographer.js`, pulling the game's board subsystem into the CMS bundle - resolves cleanly today; extract the constant to a leaf module if the CMS build gets heavy. |
| **P8** | XP + pacing | Pass 6 (well, §8): XP derivation, curves, "day in reach" | P5 | **Done 2026-09-01. THE BUTTON IS COMPLETE** - Recalculate now derives cycle times, item values, output quantities, tuning, Map checks and XP. `sim/xpPass.js`: xp/cycle = XPH curve x purpose XP factor x cycle hours, rounded, min 1, reading `purposeXpFactors` (1.0/0.5/0.3) from the dial set P5 landed. Write targets are the fields the runtime reads - recipe `xp` and token `config.xp` (S18); the dead top-level Token `xp` is flagged, not deleted. `PaceDials.jsx` (numeric pin rows over a read-only curve, per the brief's steer that the dial is the deliverable), and an **In reach** column in P7's Map table. Tests 1655 to 1708. ✅ **The mastery claim holds, integrated against the game's real `xpForLevel`:** **53.2 board-hours** 1 to 99 (plan says ~55), level 1 to 2 in **7.1 minutes** (plan says ~7), last ten levels **23.2%** of the climb (plan says ~a quarter). Three independent predictions of the design landing on the nose. Confirmed independently by the director. In-game probe: a level-1 hero on a 20s Token completed 3 cycles in 60s and was awarded exactly 3x the derived XP. ⚠️ **The printed pins reconcile, but storing ONLY them fails the band.** `700 x 1.075^(L-1)` matches all six of the plan's printed columns to under 1% - so mechanism and table are one curve. But the curve is read by straight lines, and a chord across twenty levels of a compounding curve overstates mid-segment XPH by up to ~17%, which pulled the climb to 47.4 hours, outside 50-60. The shipped pin set therefore keeps the plan's printed values **verbatim at the levels it printed them** and fills every tenth level from the same 7.5% rate; consecutive pairs grow 7.40-7.59%/level. The six printed columns are a sample, not the storage set. ⚠️ **Asymmetry worth knowing:** the XP curve does **not** bend at level 70 the way gold does. Section 13.1 flattens gold to 2%/level after 70; section 13.2's own printed 99 column is unbroken 7.5%. So "the same growth rate as the gold curve" holds only to 70. This is in the plan's numbers, and **CMS-123 already accepts it** - *"late-game XPH numbers get astronomically large (accepted - XP is display, not economy)"*. ⚠️ **F8's granularity wobble is live and documented:** level-1 GPH content over-teaches through the min-1 rounding (1 XP per 10s cycle is ~360/h against a ~210 target) and files an `xp-granularity` Info row. Accepted by the design, not a bug. ⚠️ **Implementation default:** the day-in-reach projection assumes the notional worker the curves describe - earning `gphAt(level)`, training at `xphAt(level)`, no Purpose factor on either side, gross income with no spending model. The plan pins the curves and hours/day but never says which tag the projected hero's work carries, and applying a factor would have to apply both halves of the seesaw. Stated in the UI. ⚠️ **Found, not fixed:** `dryRun.mjs` has not run under plain `node` since P7 - its `mapPass to Cartographer to DatabaseManager` chain uses `import.meta.glob`, which only Vite provides. Verification goes through vitest instead. | The owner's "run the whole simulation from Recalculate" is finished at P8, not P5: P5 gives cycle times, item values and output quantities; P6 adds tuning and refusals; P7 adds Map economics and scrap; **XP lands here.** |
| **P9** | Polish + guards | Sticky-anchor UX, progression guard, adversarial set | P6-P8 | **Done 2026-09-01. THE REWORK IS COMPLETE - tagged v0.6.0.** New `sim/checkPass.js` (progression guard + hours-first charge outliers, running last so the charge half sees the cycle times TUNE moved) and `sim/chain.js` (the sentence trail, built by re-reading pricing details rather than recomputing economics). `AnchorElections.jsx` (re-elect/dismiss), `ChainInspector.jsx`, section 14's printed dial interactions. Tests 1708 to 1755. Version bumped in five files. ✅ **The adversarial set is the phase's real deliverable:** nine broken shapes plus an enemy entry, an inert Token, an unfinished recipe and a 0-0 output range, **all in one workspace**, through the whole pipeline at once. It terminates under 5s, refuses each correctly, and is idempotent. A downcycle sitting next to a genuine cycle was told apart from it; a Token-output recipe was refused **and** blocked from anchoring the Token id it claims to produce; no NaN or Infinity anywhere - P7's lesson held under combination. ⚠️ **Two defects surfaced ONLY in combination, which is why this phase exists.** (1) An item stranded inside a recipe cycle had **no item-keyed row at all** - the cycle refusal is keyed by entity, so every item-centric reader came up empty on exactly the items a designer would be puzzling over. Fixed. (2) **A real non-idempotence:** the ANCHOR pass read `tokenType` on the way in while `recalculateEconomy` re-derived it on the way out, so a Token whose authored type disagreed with its own rules was a deferred kind on run one and a producer on run two - the same content priced two different ways on consecutive runs, converging only from run two. Plan section 11's byte-identical promise was therefore true only from the second run on, and every workspace saved before the derivation landed was in that shape. **Fixed by the director:** `tokenType` is derived in `migrateLegacyIntent`, before the passes read it - the same place and reason as the `isPrimarySource` migration. There was no design question: `tokenType` is derived with no override (owner decision Q3). Verified to move no shipped value. The test that pinned the bug is inverted, not deleted. ✅ **End to end on the owner's real content:** Recalculate over 62 tokens / 38 items / 6 recipes / 7 maps - **0 values changed**, 32 items priced, 3 sources re-tuned. Sync: `items.json` **untouched**; `maps.json` gained the derived rarity weights (10 to 100, all Common, so no share moved); `tokens.json` gained scrap values and the P2 intent seeding, plus one genuine content change that was the owner's own tag reaching the file. Game loop proven step by step: buy Map (-150g, materials consumed) to burst of 3 carrying the derived weight to place Token (refused with "Missing token: Pickaxe", resolved by adjacency) to 3 cycles awarding exactly 3x the derived XP to sell item at its derived 1g to sell Token at its derived 62g scrap, pro-rated to 57g for 93/100 uses. ⚠️ **Implementation default:** the progression guard refuses only the compounding stretch. Section 13.5 says "everywhere on the curve", but section 13.1 flattens gold to 2%/level after 70, so 13 pairs above level 72 break the ordering by construction. Making the guard absolute means moving the earn curve's late pins - a game-feel decision, not an implementation one. ⚠️ **Still fixture-proven:** the re-elect **button** has never been clicked against real content - all 32 priced items' stored elections currently agree with the rule, so the row does not appear. The write path itself is proven through the real store by five test cases including the downstream re-price. ⚠️ **A tenth sitting is worth having, and it is cleanup, not features:** the dead top-level Token `xp`, the dead `charges` field, and `connectivityAuditor`'s `token.outputs` bug - which still files 6 "Unreachable" Criticals on items the simulator prices perfectly well, and is the loudest wrong thing left on the audit tab. All three make the tool look wronger than it is to someone who did not build it. |
...
- ⚠️ **R-18 actually happened, 2026-09-01 — and the agent that did it was the
  *verifier*, not the builder.** A verification agent working on P5 opened the
  CMS, loaded a workspace that was **not** the tagged one (62 tokens and 38
  items against the shipped 39 and 18), pressed Sync, and overwrote
  `data/tokens.json`, `items.json`, `tokenRecipes.json`, **`maps.json` and
  `palettes/custom_palettes.json`** — two files the phase never touched. It was
  then cut off by an infrastructure limit before it could restore anything.
  Recovery was exactly as documented: `git checkout -- data/` back to the last
  commit, then regenerate the migration by running the CMS's own
  `recalculateEconomy` sequence headlessly, which reproduced the builder's
  output to the byte. **Two lessons.** First, "load the tagged workspace before
  syncing" is not advice, it is a precondition, and it binds every agent that
  opens the CMS — reviewers included. Second, an agent that syncs must restore
  `data/` before it stops, and cannot be relied upon to survive long enough to
  do so — so prefer verifying the sync path headlessly over clicking it.

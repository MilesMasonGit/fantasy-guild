...
| **P2.6** | **Prune the corpus to what works (R-16, R-17).** Delete the 19 recipes whose inputs nothing produces or whose context tags nothing provides, leaving 4. Retire tag-shaped item inputs; keep tag+tier tool requirements. Retire the two characterisation tests that pinned the gaps, and restore Rule 1's consume-side check to a hard assertion now that it can pass. ⚠️ **Art assets stay.** | P2.5 | Not started |

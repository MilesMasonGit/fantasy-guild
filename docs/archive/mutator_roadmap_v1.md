...
| 4 | ACTION cards & stamping | ✅ Done & verified | `02a6daf` | `src/systems/effects/MutatorStamping.js` — working a card with a `{type:'mutator', tokenId}` trait stamps the token onto matching **upcoming** slots. Charges mode = first N (surplus wasted); area mode = all matches. Forward-only, skips empty/hazard slots (~~and locked~~ — **corrected in Phase 8**, see that row). Wired in `_completeActiveSlot` after `completeWorkCycle`; `CARD_TYPES.ACTION` added to slottable types. `removes` (targeted counter) and `consumeOnUse` plumbed but dormant — content is Phase 10. Tests 220/220. Stamping logic unit-tested against the real function with real templates + browser-probed; wiring is a direct pass-through covered by existing loop tests. No gameplay effect until Phase 5 consumers. |
| 5 | Yield / Time / Cost axes | ✅ Done & verified | `23ac3ae` | `src/systems/effects/TokenAxes.js` — the three axes finally change numbers. `ModifierAggregator.resolveAxis()` runs the full §15.3 formula in one place; TokenAxes adds the §10 floors (time ≥1000ms, cost ≥1, floored only once tokens act). Wired: WORK_TIME→`StatProcessor.currentTickTime`, YIELD→`LootSystem.handleTaskReward`, INPUT_COST→`WorkProcessor.consumeInputs`. Tests 233/233. **Observable** — verified through the REAL `StatProcessor` recalc in-game: base 4000ms card → 8000ms under a ×2 Trawler, yield 3→6, and a ×2 Trawler + 5× −20% Haste floors at 1000ms (not 0). |
| 6 | Failure states | ✅ Done & verified | `ff1d2ce` + `0abe125` | **F5 fixed in its own commit** (`ff1d2ce`): new `src/systems/cards/logic/CardPreflight.js` decides the whole exchange before any of it happens, so loot + consumption are atomic. Failure reasons: `inputs` (can't afford, cost is Token-scaled) and `capacity` (bank can't store). `InventoryManager.canAccept()` is the read-only mirror of `addItem`'s guards. `module_cycle_complete` moved out of `consumeInputs` so End of Work fires on failure too (§16). Sets `card.lastFailure`, publishes `card_work_failed`. Tests 257/257. **Capacity semantics corrected after live testing** — outputs are a "pick one" cluster, so a card fails only when NONE of its outputs can be stored; the first cut wrongly failed a card if *any* single output was full. **A failed card awards nothing** (`8c571dd`, owner decision 2026-07-21): the pre-flight now gates quest progression, unified rewards (XP + items), loot and consumption alike. `applystatus` stays outside the gate (environmental, not a payoff); tool durability still burns; combat XP/loot unaffected. |
| 7 | Combat axis (Hex) | ✅ Done & verified | `f2cf2aa` | No fourth axis, as §15.13 requires: a token's `applyStatuses` is handed straight to `StatusEffectSystem.applyToEnemy` inside `applySlotTokensToCard` — the same path a weapon proc uses. `card.combat` already exists there (`CardFactory.createInstance` runs `initCombatState` first), and `applyToEnemy` no-ops on a non-combat card. Enemy DoT ticking was **already wired** (`CombatProcessor` → `tickEnemyStatuses`), so nothing new was built for it. Tests 264/264. Verified in-game through the real path: a genuine `task_cow_pasture` instance (enemy_cow, 3 HP) stamped with a Hex spawned Poisoned ×2 and died on the first DoT tick. |
| 8 | Area Anchor | ✅ Done & verified | `f8e7285` | **No bespoke machinery needed**, as the phase predicted: `buildDeckSlotsForArea` already maps an authored `slotType:'locked'` → `isLocked` and carries its `templateId`; `LoopRunner` never checks `isLocked`, so it works a locked card normally; Phase 4's area-wide mode does the broadcast. ⚠️ **Bug this phase surfaced and fixed:** `upcomingStampableSlots` was skipping `isLocked` targets. Locked governs *player editing*, not whether a card is there — §9 blueprints pin combat cards into locked anchors, so a Hex could never have reached the enemies it exists to curse. Now only empty and `hazard` slots are skipped. No impact on current content (every authored locked slot today is a hazard with no template). Tests 269/269. Verified in-game via the real `buildDeckSlotsForArea` + `stampMutatorFromCard`: an anchor at locked slot 0 broadcast to slots 1, 2 (a locked card) and 3, and never stamped itself. **No area ships an anchor yet — content is Phase 10.** |
| 9 | Token UI & tooltips | 🟡 Mostly done | `e1d0389` | New `CardTokenOverlay.jsx` (badges + FAILED! stamp) and `SlotFailures.js`. **Failure visuals VERIFIED in-game** — the stamp renders on the active card and across every failed slot in Deck Focus (§12's bottleneck view), three at once on a saturated save. **Token badges are built and unit-tested but NOT yet seen rendering**, because no mutator card is obtainable in-game yet — see the Phase 10 gap below. Condensing (×50 → one badge), source tracing, tooltip phrasing and change-events are all pinned by tests. `describeTokenEffects` + `getSlotTokenSummary` keep the logic out of the component. Tests 286/286. |
| 10 | Content catalog & counters | 🟡 Partly done | `e1d0389` + `687c304` | **§14 token catalog authored** (abundance / trawler / hex / cursed / dam) plus four mutator ACTION cards (`data/cards/action/mutators.json`) and a `MUTATOR` preset. **Acquisition wired and VERIFIED in-game:** `binderCatalog` now admits `action` (whitelist + an "Action" tab), and the Guild Hall `deckList` offers the four mutators, so they drop from the unified pack pool. Confirmed by pulling *Bountiful Rites* from a pack — it renders with its `ACTION · NATURE · GATHERING` tags, appears in the Cards drawer as available, and slots into a deck via Inspect → Add to Deck. **Owner decisions 2026-07-21:** (a) §15.7's "consumed on use" refers to CONSUMABLE cards drawing a banked item, **not** Mutator cards being destroyed — the `consumeOnUse` misreading was removed and Mutator cards are always permanent; (b) **Cursed and Dam were cut** — nothing applies a curse, so both were inert (the §15.6 `removes` primitive stays in the engine, dormant); (c) acquisition stays pack-based, crafting deferred; (d) the last-slot trap is fixed in the placer — "Add to Deck" now prefers a slot with cards after it. **REMAINING: a token badge has still not been *seen* rendering — see the note below.** |
...
> [!NOTE]
> **The one thing still unseen: a token badge rendering in the live game.**
> Everything upstream of it is verified — tokens stamp correctly, the badge
> data (condensing, sources, tooltip text) is unit-tested, and the mutator card
> is obtainable and slottable. What is missing is a single arrangement: a
> Mutator sitting *before* a matching card, with the loop then running past it.
>
> It could not be staged from this session because reordering deck slots needs
> a drag the automation environment cannot simulate (dnd-kit pointer
> sequences), and "Add to Deck" placed the card in the last slot — where
> forward-only stamping has nothing ahead of it to mark.
>
> With a mouse this is trivial: drag *Bountiful Rites* to an early slot in
> Guild Hall → Deck, ahead of a Gathering card, and let the loop reach it. The
> next Gathering cards should show a 🌾 badge. That is the check to run before
> marking Phase 9 ✅.

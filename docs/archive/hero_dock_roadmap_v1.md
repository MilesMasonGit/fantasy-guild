...
**Status:** COMPLETE — all 10 phases (0–9) done & verified · **Created:** 2026-07-21 · **Baseline:** v0.3.1 (325/325 tests green)
...
> **Result (2026-07-21):** passed, measured at 1280×800. Five heroes → five
> tabs in roster order; the dock spans exactly 712→800 (88px) at `z-200`; tabs
> are 168px wide on a 140px stride, so each overlaps its neighbour by 28px with
> the left-most on top (z 5,4,3,2,1); hovering the third raises it to z-6 above
> all others and it restores on leave. Pills went Reserve → "Whispering Woods"
> on deploy → "Injured" on wounding, live. With the banner list scrolled fully
> to the bottom, the last row clears the dock by 16px. With the Bank open, its
> content stops exactly at the dock's top edge and the dock layers above it.

> [!NOTE]
> **The in-combat pill is unit-tested, not click-tested.** `areaState.status`
> is owned by `LoopRunner` and rewritten every tick, so a hand-set
> `'in_combat'` is stamped back to `'shuffling'`/`'paused'` before it can be
> observed. That is why the pill logic lives in `dockActivity.js` as a pure
> function — all five states are covered there. The live subscription that
> feeds it is proven by the Reserve/area/Injured transitions above.
...
> **Result (2026-07-22):** all passed, driven through real dnd-kit pointer
> sequences rather than direct engine calls. Deploy assigned the hero to the
> area; recall-by-drag and the ✕ both cleared it; a Bank tile dropped on a tab
> equipped to `hand1`; clicking a slot unequipped; and a Leather Cap dragged
> from Bramble's card onto Urchin's tab ended up on **Urchin only**, with bank
> stock at 3 — the exact condition under which the naive version would have
> left it on both. An invalid drop on empty space changed no state and left the
> dock intact.

> [!NOTE]
> **dnd-kit drags CAN be automated.** A previous session concluded they could
> not, and that conclusion was too broad. `computer{left_click_drag}` is
> unavailable here because it requires a prior screenshot and screenshots time
> out against this game — but a synthetic `pointerdown` → `pointermove` (past
> the 8px threshold) → stepped `pointermove`s → `pointerup` sequence, dispatched
> with `pointerId`/`isPrimary`/`buttons` set, drives the real sensor correctly.
> See [[project-fantasy-guild-verification]] in memory for the helper.

### Bug this phase surfaced and fixed

Dropping a recalled hero anywhere on the dock initially did nothing. The strip
declares a recall target, but `smallestWithin` collision resolves to the
**smallest** droppable under the cursor — and a tab is always smaller than the
strip containing it. Every drop therefore landed on a tab, whose `accepts` only
took items, and was rejected. Tabs now accept a hero carrying `from` and
perform the recall themselves, which also reads better: you drop the hero on
the dock, not on a specific gap in it.

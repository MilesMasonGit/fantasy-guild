...
| ~~**ER-13**~~ | ⛔ **RETIRED 2026-09-09.** Statuses **do** become CMS-authorable: they become ordinary library effects, authored exactly like every other one. | Owner's ruling: *"G7 is correct, ER-13 is out of date. My goal is to be able to author these effects and others like it using our new effects system."* ER-13 existed to defer a data file and a CMS screen until the shape settled. It has settled — and holding the decision left G-7 (re-author the seven by hand) with nowhere to put them, since `statusRegistry.js` is code and the library is CMS data. Keeping both would have been the two-sources-of-effects G-1 exists to prevent. **The minimal status readout still ships with the absorb** (ER-17). |

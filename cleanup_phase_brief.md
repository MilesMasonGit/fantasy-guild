...
| `Market.test.js` | but stays modest — a Market must not make chains pointless | Economic guard rail on a Market's gold output. ⚠️ **The "under 3× the inputs / limit of 30" rule this row used to state was never set by the owner** (owner decision 21, 2026-08-20) — it was invented somewhere along the way and repeated as fact. **The real rule: a Market pays roughly a 20% premium over the Bank's sell price for the same goods**, the premium being the reward for the time and logistics a Market costs. | Deleted — behaviour changed by rework | Rewrite against the *rule* (output ≈ 1.2 × the inputs' Bank value) using `fixture_*` content. It cannot be checked against shipped content: no Market Token is authored, and every item in `data/items.json` has `sellPrice: 1` |
